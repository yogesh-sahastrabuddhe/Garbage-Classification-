# garbage_classification_fixed.py
import os
import numpy as np
import pandas as pd
...
try:
    ...
    print("⚠️  gradio not available, skipping web interface")
...
# Constants
IMG_SIZE = (380, 380)
BATCH_SIZE = 8 if tf.config.list_physical_devices('GPU') else 4
...
def simple_augmentation(img):
    """Simple augmentation for when albumentations is not available."""
    # Basic augmentations
...
def fine_tune_model(model, base_model, train_gen, val_gen, train_steps, val_steps):
    ...
    callbacks = [
        ...
        ModelCheckpoint('best_finetuned_model.keras', save_best_only=True, monitor='val_accuracy', mode='max')
    ]
...
def create_gradio_interface(model, meta_dim=3):
    """Create Gradio interface if available."""
    if not GRADIO_AVAILABLE:
        print("Gradio not available, skipping web interface")
        return None
    ...
    def classify_image(img):
        ...
        file_size = len(img.filename) if hasattr(img, 'filename') else 1000
        meta_features = np.array([[
            file_size / 1e6,  # size_normalized
            np.log(file_size + 1),  # size_log
            0  # file_number (default)
        ]]).astype('float32')
        
        # Make prediction
        pred = model.predict([np.expand_dims(img_array, 0), np.expand_dims(meta_features, 0)], verbose=0)[0]
        
        return {CLASS_NAMES[i]: float(pred[i]) for i in range(NUM_CLASSES)}
    ...
    iface = gr.Interface(
        fn=classify_image,
        inputs=gr.Image(type="filepath", label="Upload Garbage Image"),
        outputs=gr.Label(num_top_classes=3, label="Classification Results"),
        examples=examples,
        title="♻️ Enhanced Garbage Classification with EfficientNetV2L",
        description="Enhanced model with attention mechanisms and advanced techniques",
...
def main():
    ...
    print("🚀 Starting Enhanced Garbage Classification System...")
    ...
    # Save final model
    try:
        model.save('garbage_classifier_enhanced.keras')
    ...
    if GRADIO_AVAILABLE:
        ...
        try:
            iface = create_gradio_interface(model, meta_dim)
            if iface:
